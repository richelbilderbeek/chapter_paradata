...
it is the scientific paper describes what the computer code does.
...
In this paper, paradata is defined as information about how data was generated,
as shown schematically in the left panel of Figure \ref{fig:figure_1}.
Paradata does not have a clear definition yet, with common elements
such as that paradata is usually not the primary data, that it describes
a process, that it describes the way of collecting primary data, or that it
describes the way the primary data is generating.
...
experiment in general and using an example from genetic epidemiology.
A computational experiment is run by computer code
and usually collects primary data first, then runs an analysis,
which results in the generation of results.
This paper takes the standpoint that it is the computer code 
that describes this generation of results, hence computer code is paradata,
...
where the correctness of the results is relevant for healthcare.
...
of heritable (i.e. genetic) traits (i.e. a genotype), 
as well as the relation between having a certain genetic makeup 
and a certain trait (i.e. a phenotype),
where the trait can be any human property, such as weight, height,
the amount of metabolites and having a disease yes/no.
Genetic epidemiologists look for associations between genotype
and phenotype. These associations are subsequently used to 
create so-called gene panels, where the gene causing 
...
(2) the phenotypes, which are protein concentrations, as measured
from blood (\cite{enroth2014strong,enroth2015effect}).
...
consists out of many nucleotides, of which there are four types,
...
are concentrations of certain proteins as obtained from blood. 
...
Imagine that the level of IL6RA is associated with a disease,
we can create a gene panel that specifically measures
SNP \verb|rs12133641|. In this simple example, it will be easier
...
The article is metadata, as it is data that describes other data:
