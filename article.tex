...
\chapter{How to make research code useful paradata}
...
For articles in Science magazine (years 2011-2012), 12\% of 204
...
commonly abreviated as A, C, G and T respectively
\iffalse
...
154,428,283 (it is at the 154 millionth position, which happens to be located
...
From this it follow (assuming the nucleotides act independently)
...
in practice there are complex regulation mechanisms).
...
Both studies assumed a correct inference of DNA.
...
Preserving code in a reliable way is rare \cite{barnes2010publish}.
In general, within software development, 
changes at all levels are common
and embracing those changes is recommended \cite{beck2000extreme}
...
The \LaTeX code of this chapter is hosted on GitHub 
...
Hosted code commonly has a commit history,
...
However, if the code breaks tomorrow, it will go unnoticed.
...
continuous integration allows one to 
...
However, for code to work longer, one can put the code and its dependencies in
...
To repeat features of useful paradata: paradata should be comprehensive, 
...
One example is, again, when doing a t-test:
...
About two-thirds of 56 Science magazine corresponding authors 
that shared their code supply a software license \cite{stodden2018empirical}.
...
system, or data that are downloaded from online (and hence, changing) sources.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Suggestions for knowledge management}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Making code FAIR.

\subsection{Hosted code}

\subsection{CI code}

\subsection{Code in container}
...
The second step for code to be useful paradata is to add CI, 
