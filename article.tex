...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=\linewidth]{baggerly2009deriving_fig_1a_zoom.png}
  \includegraphics[width=\linewidth]{baggerly2009deriving_fig_1a.png}
  \caption{
    A published off-by-one error.
    Left: a zoom in. 
    Horizontal axis is the sample name.
    Vertical axis is UNKNOWN.
    Color in the cell denotes gene expression.
    The colors above the cells, with the dendrogram, shows
    red for non-responders (red) and responsive (blue).
    Right: visisualization of sample that shows samples are duplicated,
    as annotated by the pink rectangles. Note that, due to this, some
    samples are both non-responders and responders.
    Adapted from \cite{baggerly2009deriving}
  }
