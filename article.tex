...
tries to determine the mechanism and spread of heritable traits.
...
All these steps are described in the paper, 
but the code itself is unpublished
and the paper does not mention any way to obtain it.
The reponse rate of correnspondence authors with a request for data is around 50\% 
\cite{manca2018non, teunis2015corresponding} 
(note that for the field of emergency medicine, 
these rates are 73\% \cite{o2003email}).
...
Again, paradata should be comprehensive, documented in a useful way
...
For code to be ideal paradata, it must be usefully documented.
...
\subsubsection{... for replication}

\subsubsection{... for extending}

There exists a clear relationship between the popularity 
and the documentation effort \cite{cosentino2017systematic,hata2015characteristics}.
...
Coding errors are extremely common and contribute to the reproducibility crisis in science \cite{vable2021code}
