...
The editor is right here....

   * [ ] Re-use examples more often
...
Thanks for this guideline and I have made the text a bit more formal.
This means in practice, I have removed all uses of `I`, except in the last sentence
and I have removed all uses of `'ll`.
...
> E.g. you insinuate that code is not published that often but write also “Code is commonly published alongside of a paper, pasted as text in the Supplementary Materials” ⇒ that code would be commonly published.

This is an excellent point and I have nuanced my wording regarding this.
To examplify using your example, the rewording is something lik:
'if -in the rare case- code is supplied, it is pasted in the Supmat'.

> [ ] Be more precise in claims
...
This was indeed -in my words- a bit lazy, so thanks to you I fixed it!
I added a referenced definition (to an Open Access paper):

```
Paradata are data about the data collection process (Choumert et al., 2019using)
```
...
Thanks so much for this feedback! It has been the one most influential
...
In my field it is more common to order discussion and conclusion like I did.
To follow the book's conventions, I have ordered these chapters to 
have 'Discussion' come before 'Conclusion'.
 ...
 * What is the domain discussed (e.g. archaeology, survey research) explained in terms that make sense for a non-expert.
   ...
   * [ ] Check if genetic epidemeology is discussed in Introduction in general terms
...
> I don't mean to imply that the theme of this chapter is absolutely wrong.
   ...
   * [ ] Discuss why this is controversion
 ...
 * [ ] Add that papers to do so get more citations
 ...
 * [ ] Why did I not include that argument? It could have been mine!
...
> and maybe earlier, 
> as written deliberately to provoke thought and conversation about paradata and eScience.
 ...
 * [ ] Consider why we disagree on this: I think seeing tests, coding standard,
       and readable and short functions does help. Clarify
 ...
 * [ ] Clarify that Docker and Singularity are such environments earlier?

> Why is table S1 so far from its internal use?

Some journals require such tables to be at the bottom of the document.
I too prefer it earlier. Thanks to you, it now has a nice spot below
the Abstract.

> **[Note to the editors]**: there may be a need to have a single glossary for the book, or a consistent format/location for glossaries within each chapter.
...
Thanks to your general feedback, the title has been changed to 'The computer code of computational research is as vital paradata as the article itself'.

 * [ ] Check if title is changed again

> The are some minor disfluencies with the English language to be rectified here and this is one of them. 

 * [ ] ?

> An elaborated definition of trust/trustworthiness should also be included as that notion differs across disciplines.

 * [ ] Discuss trust/trustworthiness
...
> [is it appropriate, for example, that most credit goes to researchers for metadata about the research?]. Going back to figure 1, I also think that the metadata element is represented in a way that seems too dislocated from the rest of the research process.
